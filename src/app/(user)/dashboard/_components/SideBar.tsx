					...
					<SidebarGroupContent>
						...
						<SidebarMenu>
							{items.map((item) => (
								<SidebarMenuItem
									key={item.label}
									className='py-1 hover:py-3 transition-all duration-300 hover:bg-green-500/10 hover:text-green-500 '
								>
									<SidebarMenuButton
										asChild
										tooltip={item.label}
									...
									>
										<a
											href={item.href}
											className=' font-medium hover:py-3 hover:bg-green-500/10 hover:text-green-500 '
										>
											<span className='text-green-500'>
												{item.icon}
											</span>
											<span className='ml-2 text-white hover:text-green-500/40'>
												{item.label}
											</span>
										</a>
									</SidebarMenuButton>
